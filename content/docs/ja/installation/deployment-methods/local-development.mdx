...
フロントエンドのコードは現在、`web` ディレクトリに直接配置されています。プロジェクトのルートディレクトリから次のコマンドを実行します：
...
```bash
cd web
bun run dev   # bun を使用してフロントエンド開発サーバーを実行
```

Rsbuild は `http://localhost:3000` から最初に利用可能なポートを使用します。バックエンドがすでにポート 3000 を使用している場合、フロントエンドは通常 `http://localhost:3001` で起動します。実際の URL はターミナル出力で確認してください。開発サーバーには、API リクエストをバックエンドサービスへ転送するプロキシが設定されています。
