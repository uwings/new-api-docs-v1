...
The frontend now lives directly in the `web` directory. From the repository root, install its dependencies with:
...
```bash
cd web
bun run dev   # Run the frontend development server using bun
```

Rsbuild uses the first available port starting at `http://localhost:3000`. When the backend is already using port 3000, the frontend usually starts at `http://localhost:3001`; check the terminal output for the actual URL. The development server is configured with a proxy that forwards API requests to the backend service.
...
The built files will be generated in `web/dist`, and the backend service will automatically load these static assets.
