...
1.  **Update the primary node first**: Only the primary node runs GORM AutoMigrate, so update it first to apply the required database schema migrations
2.  **Verify the primary node**: Check its startup logs and service status to confirm that the migrations completed and the node is stable
3.  **Update secondary nodes gradually**: Update the remaining secondary nodes one by one, confirming stability after each update

This order ensures that the database schema is ready before secondary nodes start running the new version.
