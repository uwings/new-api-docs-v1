...
1. **先更新主节点**：只有主节点会执行 GORM AutoMigrate，因此应先更新主节点以完成所需的数据库结构迁移
2. **确认主节点正常**：检查启动日志和服务状态，确认数据库迁移完成且主节点稳定运行
3. **逐步更新从节点**：逐个更新其余从节点，每次确认节点稳定后再继续

这个顺序可以确保从节点运行新版本前，数据库结构已经完成更新。
