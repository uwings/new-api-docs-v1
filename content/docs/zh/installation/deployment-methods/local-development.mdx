...
```bash
cd web
bun run dev   # 使用 bun 运行前端开发服务器
```

Rsbuild 会从 `http://localhost:3000` 开始使用第一个可用端口。后端已占用 3000 端口时，前端通常会运行在 `http://localhost:3001`；实际地址请查看终端输出。开发服务器已配置代理，会将 API 请求转发到后端服务。
